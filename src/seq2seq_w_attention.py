"""
This tutorial demonstrates how to train a seq2seq model with an attention mechanism for machine translation.
While the tutorial focuses on integrating the attention mechanism, to keep things simple, it employs a basic
greedy approach for decoding rather than more advanced techniques like beam search.
"""
...
"""
A sequence-to-sequence (seq2seq) model, like the one we'll use for machine translation, 
consists of a few main components: the Encoder, the Decoder, and Attention. 
...
between the Encoder and Decoder. Moreover, we also create a class for Attention mechanism.
"""
...
"""
The Attention mechanism helps the decoder focus on different parts of the input
sequence when generating each word of the output sequence.
"""
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        """We use only the top layer hidden state of the encoder"""
...
2. The Decoder generates the output sequence (in this case, a French sentence) one token at a time,
   using the attention mechanism to focus on relevant parts of the input sequence.
"""
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hidden_dim, n_layers=1, attention=None):
        super(Decoder, self).__init__()
        self.embedding = nn.Embedding(output_dim, emb_dim)
        self.lstm = nn.LSTM(emb_dim + hidden_dim, hidden_dim, n_layers, batch_first=True)
...
"""
The Seq2Seq class orchestrates the entire sequence-to-sequence translation process, integrating the
encoder and decoder.
"""
...
"""Instantiate the Encoder, Attention, Decoder, and Seq2Seq model, then move the model to the specified device."""
attention = Attention(512)
encoder = Encoder(len(tok2ix['src_lang']), 256, 512, 2).to(device)
decoder = Decoder(len(tok2ix['trg_lang']), 256, 512, 2, attention=attention).to(device)
...
Example 1:
Source: this made me sad
Expected: cela m'a rendue triste
Predicted: ãa m'a rendu triste
------------------------------
Example 2:
Source: is she all right
Expected: <UNK> bien 
Predicted: se porte-t-elle bien 
------------------------------
Example 3:
Source: i could help
Expected: je pourrais aider
Predicted: je pouvais aider
------------------------------
Example 4:
Source: i didn't see you
Expected: je ne vous ai pas vues
Predicted: je ne vous ai pas vue
------------------------------
Example 5:
Source: i fear no one
Expected: je ne crains personne
Predicted: je ne fais un
------------------------------
Example 6:
Source: we're canadians
Expected: nous sommes <UNK>
Predicted: nous sommes canadiennes
------------------------------
Example 7:
Source: i can manage
Expected: je peux me dã©brouiller
Predicted: je peux le maison
------------------------------
Example 8:
Source: let's ask him
Expected: allons lui demander
Predicted: demandons-lui 
------------------------------
Example 9:
Source: i'm too busy
Expected: je suis trop occupã©
Predicted: je suis trop affairã©e
------------------------------
Example 10:
Source: i thought so
Expected: je m'en suis <UNK>
Predicted: je l'ai ai bien
------------------------------
Example 11:
Source: i <UNK> cookies
Expected: j'ai confectionnã© des biscuits
Predicted: je me suis d'accord
------------------------------
Example 12:
Source: i really mean it
Expected: je suis sã©rieuse
Predicted: je suis sã©rieux
------------------------------
Example 13:
Source: who's coming
Expected: qui vient 
...
------------------------------
Example 14:
Source: look at your map
Expected: regardez votre carte
Predicted: regardez ta carte
------------------------------
Example 15:
Source: you're not dead
Expected: tu n'es pas mort
Predicted: vous n'ãªtes pas morte
------------------------------
"""
...
covers preprocessing techniques that might not be necessary for this tutorial. The tutorial also uses separate
